...
export function generateStaticParams() {
  return getSeoLandingPagesByPrefix("/de/amsterdam/").map((page) => ({
    seoSlug: page.href.replace("/de/amsterdam/", "")
  }));
}

export async function generateMetadata({ params }: SeoSlugPageProps): Promise<Metadata> {
  const { seoSlug } = await params;
  const page = getSeoLandingPage(`/de/amsterdam/${seoSlug}`);
...
export default async function GermanAmsterdamSeoSlugPage({ params }: SeoSlugPageProps) {
  const { seoSlug } = await params;
  const page = getSeoLandingPage(`/de/amsterdam/${seoSlug}`);
